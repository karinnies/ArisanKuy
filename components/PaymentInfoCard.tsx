...
interface Props {
  // Fix: Changed props to accept individual strings instead of AppSettings object to match App.tsx usage
  waAdmin?: string;
  rekening?: string;
  iuran?: number;
}

const PaymentInfoCard: React.FC<Props> = ({ waAdmin, rekening, iuran }) => {
  const WA_NUMBER = waAdmin || "6281234567890";
  const REK_INFO = rekening || "BCA 8820-XXXX-XX";
  ...
  return (
    <div className="bg-white rounded-[1.5rem] p-6 shadow-sm border border-slate-100 flex flex-col h-full">
      ...
      <div className="space-y-4 flex-1 flex flex-col">
        <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex items-center gap-4 mb-auto">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex justify-between items-center gap-2">
              <span className="font-bold text-slate-700 text-sm truncate">{REK_INFO}</span>
              <button 
                onClick={() => { navigator.clipboard.writeText(REK_INFO); alert("Disalin!"); }}
                className="text-slate-300 hover:text-rose-400 transition-colors text-xs shrink-0"
              >
                Salin
              </button>
            </div>
            {iuran && (
              <p className="text-[10px] font-black text-rose-500 mt-1">Iuran: Rp {iuran.toLocaleString()}</p>
            )}
          </div>
        </div>

        <div className="pt-4">
          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-3 text-center">Sudah Bayar? Konfirmasi di sini</p>
          <a 
            href={waUrl}
            target="_blank"
